...
func (s *Session) addManagedLocked(ctx context.Context, hash metainfo.Hash, spec *torrent.TorrentSpec, data []byte, uri string, logFailure func(error)) (*TorrentView, error) {
	...
	if data != nil {
		// The guarded publish refuses a torrent that would rename an older video
		// out from under its managed subtitle before writing anything.
		publishedFinal, _, err = s.publishGuardedMetainfo(ctx, hash, data)
		if err == nil {
...
func (s *Session) startMetadataFetch(hash metainfo.Hash, st *Torrent) {
	...
	go func() {
		...
		// A resolved magnet is taking a root name for the first time, so it
		// takes the namespace serialization an add takes: the guard inside
		// publishGuardedMetainfo decides before any file exists on disk.
		s.rootNamespaceMu.Lock()
		defer s.rootNamespaceMu.Unlock()
		unlock := s.lockHash(hash)
		defer unlock()
		...
		created, spec, err := s.publishGuardedMetainfo(ctx, hash, buf.Bytes())
		if err == nil {
			entry := cloneRegistryEntry(s.states[hash])
			if entry == nil {
				s.mu.Unlock()
