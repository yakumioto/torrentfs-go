...
2. 在 Tampermonkey 中导入 `userscript/torrentfs.user.js`。脚本显式声明 `@sandbox DOM`，并通过注入式 page bridge/postMessage 读取 M-Team 页面数据；它使用已批准的 `@connect *` 以支持任意用户配置地址，但运行时只请求当前配置的 TorrentFS base 和允许的 M-Team 下载 host；没有远程 `@require`。
